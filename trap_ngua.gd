...
func _on_trigger_entered(body):
	if body is CharacterBody2D and not has_triggered:
		# Kiểm tra xem nhân vật có đang đứng trên platform không
		if body.is_on_floor() and body.get_floor_normal().dot(Vector2.UP) > 0.9:
			print("Player is standing on platform, triggering rotation!")
			has_triggered = true
			rotate_platform()
